...
## Why does Tokmak use HTML/CSS for rendering in the browser?
...
Additionally, we can rely on optimized CSS layout algorithms where possible. This also unlocks more use-cases for Tokmak, such as
[static HTML generation](https://github.com/TokmakUI/TokmakPublish) and [server-side rendering](https://github.com/TokmakUI/TokmakVapor).

At the same time, Tokmak has [a new layout system in development](https://github.com/TokmakUI/Tokmak/pull/472) that accesses
DOM directly for layout calculations, bypassing CSS for a lot (or potentially all) of its algorithms.

## Does the word Tokmak mean anything? Why is it called this?

The project was originally inspired by [React](https://reactjs.org), which utilizes a model of an atom in its logo,
apparently as a reference to nuclear reactors. [Токамак](https://en.wikipedia.org/wiki/Tokmak) is a nuclear fusion reactor, and 
...
API](https://github.com/swiftwasm/Tokmak/blob/0.1.2/README.md), Tokmak for iOS/macOS was [sherlocked](https://en.wikipedia.org/wiki/Sherlock_(software)#Sherlocked_as_a_term) by
