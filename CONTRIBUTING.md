...
Tokmak is built with modularity in mind, providing a multi-platform `TokmakCore` module and
separate modules for platform-specific renderers. Currently, the only available renderer modules are
`TokmakDOM` and `TokmakStaticHTML`, the latter can be used for static websites and server-side
...
Tokmak users only need to import a renderer module they would like to use, while
`TokmakCore` is hidden as an "internal" `Tokmak` package target. Unfortunately, Swift does not
allow us to specify that certain symbols in `TokmakCore` are private to a package, but they need to
...
1. If a symbol is restricted to a module and has no `public` access control, no need for an
   underscore.
2. If a symbol is part of a public renderer module API (e.g. `TokmakDOM`), no need for an
   underscore, users may use those symbols directly, and it is re-exported from `TokmakCore` by the
...
3. If a function or a type have `public` on them only by necessity to make them available in
   `TokmakDOM`, but unavailable to users (or not intended for public use), underscore is needed to
   indicate that.
...
only `TokmakStaticHTML`, single-page apps would use `TokmakDOM`, maybe in conjuction with
`TokmakStaticHTML` for pre-rendering. As we'd like to try to implement a native renderer for
Android at some point, probably in a separate `TokmakAndroid` module, Android apps would use
`TokmakAndroid` with no need to be aware of any of the web modules.

### Testing

Tokmak uses [SnapshotTesting](https://github.com/pointfreeco/swift-snapshot-testing) library to
make sure that HTML and layout generated by renderers stay consistent. To run the test suite on macOS
you should use this command:

```sh
swift build --product TokmakPackageTests && `xcrun --find xctest` .build/debug/TokmakPackageTests.xctest
```
