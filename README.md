<img alt="Tokmak logo" src="Sources/TokmakDemo/logo-header.png" width="640px"/>

## SwiftUI-compatible framework for building browser apps with WebAssembly

[![CI status](https://github.com/swiftwasm/Tokmak/workflows/CI/badge.svg?branch=main)](https://github.com/TokmakUI/Tokmak/actions?query=workflow%3ACI) [![Discord](https://img.shields.io/discord/780838335798706197?label=Discord)](https://discord.gg/ashJW8T8yp)

At the moment Tokmak implements a very basic subset of SwiftUI. Its DOM renderer supports a few
...
goal of Tokmak is to implement as much of SwiftUI API as possible and to provide a few more helpful
...
[issues](https://github.com/swiftwasm/Tokmak/issues) and
[PRs](https://github.com/swiftwasm/Tokmak/pulls) to get more details about the current status, or
[create a new issue](https://github.com/swiftwasm/Tokmak/issues/new) to let us prioritize the
...
below](https://github.com/swiftwasm/Tokmak#arbitrary-html)), so pull requests are very welcome!
Don't forget to check [the "Contributing"
section](https://github.com/swiftwasm/Tokmak#contributing) first.
...
Tokmak API attempts to resemble SwiftUI API as much as possible. The main difference is
that you use `import TokmakShim` instead of `import SwiftUI` in your files. The former makes
your views compatible with Apple platforms, as well as platforms supported by Tokmak (currently
...
`HTML` doesn't support event listeners, and is declared in the `TokmakStaticHTML` module, which `TokmakDOM` re-exports. The benefit of `HTML` is that you can use it for static rendering in libraries like [TokmakVapor](https://github.com/TokmakUI/TokmakVapor) and [TokmakPublish](https://github.com/TokmakUI/TokmakPublish).

Another option is the `DynamicHTML` view provided by the `TokmakDOM` module, which has a `listeners` property with a corresponding initializer parameter. You can pass closures that can handle `onclick`, `onmouseover` and other DOM events for you in the `listeners` dictionary. Check out [MDN docs](https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers) for the full list.
...
### For users of apps depending on Tokmak
...
Tokmak relies on [`carton`](https://carton.dev) as a primary build tool. As a part of these steps
you'll install `carton` via [Homebrew](https://brew.sh/) on macOS (unfortunately you'll have to build
it manually on Linux). Assuming you already have Homebrew installed, you can create a new Tokmak
...
You can also clone this repository and run `carton dev --product TokmakDemo` in its root
...
Open `Package.swift` of your project that depends on Tokmak with Xcode and build it for macOS.
...
All relevant modules of Tokmak (including `TokmakDOM`) should compile on macOS. You may see issues
with `TokmakShim` on macOS Catalina, where relevant SwiftUI APIs aren't supported, but replacing
`import TokmakShim` with `import TokmakDOM` should resolve the issue until you're able to update
to macOS Big Sur.

If you stumble upon code in Tokmak that doesn't build on macOS and prevents syntax highlighting or
autocomplete from working in Xcode, please [report it as a
bug](https://github.com/TokmakUI/Tokmak/issues/new).
...
implemented in Tokmak you may learn more how SwiftUI may work under the hood.

Updating our [documentation](https://github.com/TokmakUI/Tokmak/tree/main/docs) and taking on [the starter
bugs](https://github.com/TokmakUI/Tokmak/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22)
...
Conduct](https://github.com/swiftwasm/Tokmak/blob/main/CODE_OF_CONDUCT.md).
...
- Thanks to everyone who developed [React](https://reactjs.org/) with its [reconciler/renderer
  architecture](https://reactjs.org/docs/codebase-overview.html#renderers) that inspired Tokmak
  in the first place.
...
SwiftUI is a trademark owned by Apple Inc. Software maintained as a part of the Tokmak project
is not affiliated with Apple Inc.

## License

Tokmak is available under the Apache 2.0 license.
Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the [LICENSE](https://github.com/swiftwasm/Tokmak/blob/main/LICENSE) file for
more info.
